...
class Thread_A_Task(QThread):
    def __init__(self, parent): # parent는 WindowClass에서 잔달하는 self이다.(WindowClass의 인스턴스)
        ...
        self.parent = parent
    def run(self):
        self.parent.textBrowser.append('A 업무를 시작합니다')
        for i in range(0, 101, 10): 
            self.parent.textBrowser.append('A 업무 진행률 {}%'.format(i)) 
            time.sleep(2)
        self.parent.textBrowser.append('A 업무 종료')
...
class Thread_B_Task(QThread):
    def __init__(self, parent): # parent는 WindowClass에서 잔달하는 self이다.(WindowClass의 인스턴스)
        ...
        self.parent = parent
    def run(self):
        self.parent.textBrowser.append('B 업무를 시작합니다')
        for i in range(0, 101, 10): 
            self.parent.textBrowser.append('B 업무 진행률 {}%'.format(i)) 
            time.sleep(2)
        self.parent.textBrowser.append('B 업무 종료')
        
form_class = uic.loadUiType("./PyQt_workspace/PyQt/QThreadTest3.ui")[0] 
